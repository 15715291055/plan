...
function App() {
  ...
  async function addTask(title: string, minutes: number, course: string, strategy: ReplanStrategy = 'minimal_change', deadlineIso?: string | null, difficulty?: number, type?: string) {
    try {
      const created = await createTaskRecord({ title, minutes, course: course || '未分类', deadlineIso, difficulty, type, source: 'temporary' })
  ...
  async function editTask(id: string, title: string, minutes: number, course: string, deadlineIso?: string | null, difficulty?: number, type?: string) {
    ...
    setTasks(current => current.map(task => task.id === id ? { ...task, title, minutes, course: course || '未分类', deadlineIso: deadlineIso ?? undefined, deadline: deadlineIso ? new Date(deadlineIso).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '未设置', difficulty: difficulty ? difficulty >= 4 ? '较难' : difficulty >= 3 ? '中等' : '简单' : task.difficulty, type: type ?? task.type } : task))
    try { await updateTaskRecord(id, { title, minutes, course: course || '未分类', deadlineIso, difficulty, type }); setEditingTask(null); setToast(dataSource === 'supabase' ? '任务已更新到云端' : '任务已更新') } catch (error) { setTasks(previous); setToast(error instanceof Error ? `保存失败：${error.message}` : '保存失败，请重试') }
  }
  ...
  async function runReplan(strategy: ReplanStrategy = 'minimal_change', taskList = tasks, availabilityOverride = availability, fixedEventsOverride = fixedEvents) {
    ...
    try {
      ...
      const result = buildSchedule(taskList, planningAvailability, fixedEventsOverride, scheduleItems, { strategy, blockMinutes: preferences.defaultBlockMinutes, bufferRatio: preferences.bufferRatio })
...
function SettingsView({ windowOpacity, onWindowOpacityChange, visualStyle, onVisualStyleChange, profile, availability, fixedEvents, preferences, deepSeekKey, hasSavedDeepSeekKey, onDeepSeekKeyChange, onSaveDeepSeekKey, onDeleteDeepSeekKey, onSavePreferences, onSaveProfile, onImportSchedule, onAddAvailability, onUpdateAvailability, onDeleteAvailability, onAddFixedEvent, onUpdateFixedEvent, onDeleteFixedEvent, onAuthChange }: { windowOpacity: number; onWindowOpacityChange: (value: number) => void; visualStyle: 'style-1' | 'style-2' | 'style-3'; onVisualStyleChange:(value: 'style-1' | 'style-2' | 'style-3')=>void; profile: UserProfile; availability: AvailabilityRule[]; fixedEvents: FixedEvent[]; preferences: UserPreferences; deepSeekKey: string; hasSavedDeepSeekKey: boolean; onDeepSeekKeyChange:(value:string)=>void; onSaveDeepSeekKey:()=>Promise<void>; onDeleteDeepSeekKey:()=>Promise<void>; onSavePreferences:(input: UserPreferences)=>Promise<void>; onSaveProfile:(input: UserProfile)=>Promise<void>; onImportSchedule:()=>void; onAddAvailability:(input:{weekday:number;startTime:string;endTime:string})=>Promise<void>; onUpdateAvailability:(id:string,input:{weekday:number;startTime:string;endTime:string})=>Promise<void>; onDeleteAvailability:(id:string)=>Promise<void>; onAddFixedEvent:(input:{title:string;startTime:string;endTime:string;recurrenceRule?:string})=>Promise<void>; onUpdateFixedEvent:(id:string,input:{title:string;startTime:string;endTime:string;recurrenceRule?:string})=>Promise<void>; onDeleteFixedEvent:(id:string)=>Promise<void>; onAuthChange:()=>Promise<void> }) {
  ...
  async function saveSettings() { await Promise.all([onSavePreferences({ defaultBlockMinutes: blockMinutes, bufferRatio: bufferPercent / 100, autoLog }), onSaveProfile({ displayName, avatarUrl }), deepSeekKey.trim() ? onSaveDeepSeekKey() : Promise.resolve()]) }
...
function AddTaskModal({ initial, courses, onClose, onAdd, onUpdate }: { initial?: Task; courses: Course[]; onClose:()=>void; onAdd?: (title:string, minutes:number, course:string, strategy: ReplanStrategy, deadlineIso?: string | null, difficulty?: number, type?: string)=>void; onUpdate?: (id:string, title:string, minutes:number, course:string, deadlineIso?: string | null, difficulty?: number, type?: string)=>void }) { const [title, setTitle] = useState(initial?.title ?? ''); const [minutes, setMinutes] = useState(initial?.minutes ?? 30); const [course, setCourse] = useState(initial?.course === '未分类' ? '' : initial?.course ?? ''); const [deadline, setDeadline] = useState(initial?.deadlineIso?.slice(0, 16) ?? ''); const [difficulty, setDifficulty] = useState(initial?.difficulty === '较难' ? 4 : initial?.difficulty === '简单' ? 2 : 3); const [type, setType] = useState(initial?.type ?? '学习'); const [strategy, setStrategy] = useState<ReplanStrategy>('minimal_change'); const editing = Boolean(initial); const savedCourse = course || '未分类'; return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}><div className="modal"><div className="modal-head"><div><span className="eyebrow">{editing ? '编辑任务' : '快速添加'}</span><h2>{editing ? '修改任务' : '新建临时任务'}</h2></div><button className="icon-btn" onClick={onClose}><X size={18} /></button></div><label>任务名称<input autoFocus placeholder="例如：整理课堂笔记" value={title} onChange={e=>setTitle(e.target.value)} /></label><div className="form-grid"><label>课程<span className="field-optional">可选</span><select className={!course ? 'is-placeholder' : ''} value={course} onChange={e=>setCourse(e.target.value)}><option value="">暂不选择课程</option>{courses.map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label><label>预计时长<select value={minutes} onChange={e=>setMinutes(Number(e.target.value))}><option value="25">25 分钟</option><option value="30">30 分钟</option><option value="50">50 分钟</option><option value="90">90 分钟</option></select></label><label>截止时间<input type="datetime-local" value={deadline} onChange={e=>setDeadline(e.target.value)} /></label><label>难度<select value={difficulty} onChange={e=>setDifficulty(Number(e.target.value))}><option value="1">简单</option><option value="3">中等</option><option value="4">较难</option></select></label></div><label>任务类型<input value={type} onChange={e=>setType(e.target.value)} placeholder="例如：作业、复习、背诵" /></label>{!editing && <label>加入计划方式<select value={strategy} onChange={e=>setStrategy(e.target.value as ReplanStrategy)}><option value="preserve">保持原计划</option><option value="minimal_change">尽量少改动</option><option value="urgent">紧急插入</option></select></label>}<div className="modal-footer"><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" disabled={!title.trim()} onClick={()=>editing ? onUpdate?.(initial!.id, title, minutes, savedCourse, deadline ? new Date(deadline).toISOString() : null, difficulty, type) : onAdd?.(title, minutes, savedCourse, strategy, deadline ? new Date(deadline).toISOString() : null, difficulty, type)}>{editing ? <Check size={16} /> : <Plus size={16} />}{editing ? '保存修改' : '加入今日计划'}</button></div></div></div> }
