...
import {
  ...
  type DailyCapacityOverride,
  createCapacityProfile,
  updateCapacityProfile,
  deleteCapacityProfile,
  upsertDailyCapacityOverride,
  deleteDailyCapacityOverride,
} from './lib/data'
...
type VisualStyle = 'style-1' | 'style-2' | 'style-3'

function BackgroundStyleChooser({ visualStyle, onChange }: { visualStyle: VisualStyle; onChange: (value: VisualStyle) => void }) {
  return <section className="panel background-style-panel"><div className="panel-head"><div><h2>背景风格</h2><span className="panel-caption">风格 1 保留细雪动态，风格 2、3 使用你提供的图片</span></div></div><div className="background-style-options">{[
    ['style-1', '风格 1', '雪日海港'],
    ['style-2', '风格 2', '海港晴空'],
    ['style-3', '风格 3', '海港夜景'],
...
function App() {
  ...
  const [dailyCapacityOverrides, setDailyCapacityOverrides] = useState<DailyCapacityOverride[]>([])
  ...
  const [visualStyle, setVisualStyle] = useState<VisualStyle>(() => { try { const value = localStorage.getItem('study-visual-style'); return value === 'style-4' ? 'style-3' : value === 'style-3' ? 'style-2' : value === 'style-2' ? 'style-1' : value === 'style-1' ? 'style-1' : 'style-1' } catch { return 'style-1' } })
  ...
  const [adjustmentEvents, setAdjustmentEvents] = useState<import('./lib/habits').ScheduleAdjustmentEvent[]>([])
  ...
  async function refreshWorkspace() {
    ...
    try {
      ...
      setAdjustmentEvents(workspace.adjustmentEvents)
      setStudyLogs(workspace.studyLogs)
      setPreferences(workspace.preferences)
      setCapacityProfiles(workspace.capacityProfiles)
      setDailyCapacityOverrides(workspace.dailyCapacityOverrides)
  ...
  useEffect(() => { if (!loading && dataSource === 'local') persistLocal(tasks, courses, { availability, fixedEvents, scheduleItems, materials, studyLogs, preferences, capacityProfiles, dailyCapacityOverrides }) }, [tasks, courses, availability, fixedEvents, scheduleItems, materials, studyLogs, preferences, capacityProfiles, dailyCapacityOverrides, loading, dataSource])
  useEffect(() => {
  ...
  scheduleItems.filter(item => { const start = new Date(item.startTime); const end = new Date(item.endTime); const dayStart = new Date(); dayStart.setHours(0,0,0,0); const dayEnd = new Date(dayStart); dayEnd.setDate(dayEnd.getDate()+1); return start < dayEnd && end > dayStart }).forEach(item => {
  ...
  async function toggleTask(id: string) {
    ...
    setTasks(current => current.map(t => t.id === id ? { ...t, status: nextStatus, completedAt: nextStatus === 'done' ? new Date().toISOString() : null } : t))
    try { await updateTaskStatus(id, nextStatus); if (nextStatus === 'done') setScheduleItems(current => current.filter(item => item.taskId !== id || !isFutureScheduleItem(item, new Date()))) } catch (error) {
  ...
  async function editScheduleItem(before: ScheduleItem, startTime: string, endTime: string, locked: boolean) {
    const task = tasks.find(item => item.id === before.taskId)
    if (!task) { setToast('找不到该计划块对应的任务'); return }
    const updated: ScheduleItem = { ...before, startTime, endTime, locked, source: 'manual', manuallyAdjustedAt: new Date().toISOString() }
    try {
      const event = await saveManualScheduleItem(before, updated, task, preferences.habitLearningEnabled !== false)
      setScheduleItems(current => current.map(item => item.id === before.id ? updated : item))
      if (event) setAdjustmentEvents(current => [...current, event])
      setEditingScheduleItem(null)
      setToast('计划块已调整，后续重排会保留人工安排')
    } catch (error) { setToast(error instanceof Error ? `保存失败：${error.message}` : '保存失败，请重试') }
  }
  ...
  async function addCapacityProfile(input: Omit<CapacityProfile, 'id' | 'createdAt' | 'updatedAt'>) { const created = await createCapacityProfile(input); setCapacityProfiles(current => [...current, created]); setToast('特殊模式已保存；点击重新排程后应用') }
  async function editCapacityProfile(id: string, input: Omit<CapacityProfile, 'id' | 'createdAt' | 'updatedAt'>) { const updated = await updateCapacityProfile(id, input); setCapacityProfiles(current => current.map(item => item.id === id ? updated : item)); setToast('特殊模式已更新；点击重新排程后应用') }
  async function removeCapacityProfile(id: string) { await deleteCapacityProfile(id); setCapacityProfiles(current => current.filter(item => item.id !== id)); setToast('特殊模式已删除') }
  async function saveDailyOverride(input: Omit<DailyCapacityOverride, 'id' | 'createdAt' | 'updatedAt'>) { const saved = await upsertDailyCapacityOverride(input); setDailyCapacityOverrides(current => [...current.filter(item => item.date !== saved.date), saved].sort((a, b) => a.date.localeCompare(b.date))); setToast('单日容量已保存；点击重新排程后应用') }
  async function removeDailyOverride(id: string) { await deleteDailyCapacityOverride(id); setDailyCapacityOverrides(current => current.filter(item => item.id !== id)); setToast('单日容量已删除') }
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${visualStyle}`} data-transparent-panels={windowOpacity === 0} data-api-key-saved={hasSavedDeepSeekKey} style={{ '--window-opacity': windowOpacity / 100 } as React.CSSProperties}>
    ...
    {editingScheduleItem && <ScheduleItemModal item={editingScheduleItem} task={tasks.find(task => task.id === editingScheduleItem.taskId)} onClose={() => setEditingScheduleItem(null)} onSave={(startTime, endTime, locked) => editScheduleItem(editingScheduleItem, startTime, endTime, locked)} />}
...
function TodayView({ profile, tasks, allTasks, availability, fixedEvents, scheduleItems, preferences, capacityProfiles, dailyCapacityOverrides, progress, totalMinutes, toggleTask, startTimer, timerTask, timerLabel, stopTimer, onAdd, replanning, onReplan }: { profile: UserProfile; tasks: TodayTask[]; allTasks: Task[]; availability: AvailabilityRule[]; fixedEvents: FixedEvent[]; scheduleItems: ScheduleItem[]; preferences: UserPreferences; capacityProfiles: CapacityProfile[]; dailyCapacityOverrides: DailyCapacityOverride[]; progress: number; totalMinutes: number; toggleTask: (id:string)=>void; startTimer:(id:string)=>void; timerTask:string|null; timerLabel:string; stopTimer:()=>void; onAdd:()=>void; replanning:boolean; onReplan:()=>void }) {
  ...
  const capacity = getDailyCapacityBreakdown({ date: now, now, preferences, profiles: capacityProfiles, overrides: dailyCapacityOverrides, unavailableRules: availability, fixedEvents, scheduleItems })
  const dailyCapacity = capacity.effectiveCapacityMinutes
  const scheduledMinutes = capacity.scheduledMinutes
  const loadRatio = dailyCapacity > 0 ? scheduledMinutes / dailyCapacity : scheduledMinutes > 0 ? Infinity : 0
  ...
  return <div className="page"><div className="page-head"><div className="daily-welcome"><div className="eyebrow">{now.toLocaleDateString('zh-CN', { weekday: 'long', month: 'long', day: 'numeric' })}</div><h1>{greetingFor(now)}，{profile.displayName} <span className="wave">✦</span></h1><p className="subhead daily-quote" lang="en"><q>{quote.text}</q> <span className="quote-author">— {quote.author}</span></p></div><div className="head-actions"><button className="button secondary" onClick={onReplan} disabled={replanning}><RefreshCw size={16} className={replanning ? 'spin' : ''} />{replanning ? '正在排程...' : '重新排程'}</button><button className="button primary" onClick={onAdd}><Plus size={17} />添加任务</button></div></div>
    <div className="stat-grid"><div className="stat-card accent"><div className="stat-top"><span>今日学习</span><Clock3 size={17} /></div><strong>{Math.floor(totalMinutes / 60)}<small>h</small> {totalMinutes % 60}<small>m</small></strong><div className="stat-meta"><span>计划总时长</span><span className="trend">{tasks.length ? `${tasks.length} 项` : '暂无任务'}</span></div></div><div className="stat-card"><div className="stat-top"><span>完成进度</span><span className="mini-ring">{progress}%</span></div><strong>{done}<small> / </small>{allTasks.length}<small> 项</small></strong><div className="progress-line"><i style={{ width: `${progress}%` }} /></div></div><div className="stat-card"><div className="stat-top"><span>今日容量</span><Zap size={17} /></div><strong>{Math.floor(dailyCapacity / 60)}<small>h</small> {dailyCapacity % 60}<small>m</small></strong><div className="stat-meta"><span>已安排 {scheduledMinutes}m</span><span className="neutral">可继续 {capacity.allocatableNowMinutes}m</span></div>{(capacity.source !== 'default' || capacity.constrainedByTimeWindow) && <div className="capacity-source-badge">{capacity.source === 'override' ? '单日调整' : capacity.source === 'profile' ? capacity.sourceName ?? '特殊模式' : '受时间窗口限制'}</div>}</div><div className="stat-card"><div className="stat-top"><span>计划负荷</span><span className="load-dot" /></div><strong className="load-value">{loadRatio === 0 ? '休息日' : capacity.overloaded ? '超载' : loadRatio > .9 ? '偏高' : loadRatio > .65 ? '适中' : '轻松'}</strong><div className={`load-bar ${capacity.overloaded ? 'overloaded' : ''}`}><i style={{ width: `${Math.min(100, Number.isFinite(loadRatio) ? loadRatio * 100 : 100)}%` }} /></div><div className="stat-meta"><span>{Number.isFinite(loadRatio) ? `${Math.round(loadRatio * 100)}% 已安排` : '超载'}</span></div></div></div>
...
function WeekView({ tasks, scheduleItems, fixedEvents, scheduleChanges, dataSource, onBack, replanning, onReplan, onEditScheduleItem }: { tasks:Task[]; scheduleItems: ScheduleItem[]; fixedEvents: FixedEvent[]; scheduleChanges: Array<{ type: string; taskId: string; from?: string; to?: string; reason?: string }>; dataSource: 'supabase' | 'local'; onBack:()=>void; replanning:boolean; onReplan:()=>void; onEditScheduleItem:(item: ScheduleItem)=>void }) {
  const [weekOffset, setWeekOffset] = useState(0)
  const weekStart = mondayOf(new Date(), weekOffset)
  const blocks = useMemo<Array<{ day: number; start: number; duration: number; title: string; course: string; color: string; item?: ScheduleItem }>>(() => {
    const fixedBlocks = fixedEvents.map(event => {
      const date = new Date(event.startTime)
      const day = Math.floor((date.getTime() - weekStart.getTime()) / 86400000)
      return { day, start: date.getHours() + date.getMinutes() / 60, duration: Math.max((new Date(event.endTime).getTime() - date.getTime()) / 3600000, .4), title: event.title, course: '固定课程', color: '#aeb9c7' }
    }).filter(block => block.day >= 0 && block.day < 7)
    if (scheduleItems.length) return [...scheduleItems.map(item => {
      const task = tasks.find(t => t.id === item.taskId)
      const date = new Date(item.startTime)
      const day = Math.floor((date.getTime() - weekStart.getTime()) / 86400000)
      return { day, start: date.getHours() + date.getMinutes() / 60, duration: Math.max((new Date(item.endTime).getTime() - date.getTime()) / 3600000, .4), title: task?.title ?? '学习任务', course: task?.course ?? '未分类', color: task?.color ?? '#8793a1', item }
    }).filter(block => block.day >= 0 && block.day < 7), ...fixedBlocks]
...
function ScheduleItemModal({ item, task, onClose, onSave }: { item: ScheduleItem; task?: Task; onClose: () => void; onSave: (startTime: string, endTime: string, locked: boolean) => Promise<void> }) {
  const [start, setStart] = useState(() => toDateTimeLocal(item.startTime))
  const [end, setEnd] = useState(() => toDateTimeLocal(item.endTime))
  ...
  async function save() {
    ...
    if (!start || !end || Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime()) || endDate <= startDate) { setError('请填写有效的开始和结束时间'); return }
    setSaving(true); setError('')
    try { await onSave(startDate.toISOString(), endDate.toISOString(), locked) } catch (reason) { setError(reason instanceof Error ? reason.message : '保存失败，请重试') } finally { setSaving(false) }
  }
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}><div className="modal schedule-item-modal"><div className="modal-head"><div><span className="eyebrow">编辑计划块</span><h2>{task?.title ?? '学习任务'}</h2></div><button className="icon-btn" onClick={onClose} aria-label="关闭"><X size={18} /></button></div><p className="panel-caption">可调整到任意日期和时间；保存后会标记为人工安排。</p><div className="schedule-item-fields"><label>开始时间<input type="datetime-local" value={start} onChange={event => setStart(event.target.value)} /></label><label>结束时间<input type="datetime-local" value={end} onChange={event => setEnd(event.target.value)} /></label></div><label className="switch-label"><button type="button" className={`switch ${locked ? 'on' : ''}`} onClick={() => setLocked(value => !value)} aria-pressed={locked}><i /></button><span>{locked ? '锁定计划块，重新排程时保留' : '不锁定，但仍保留人工时间安排'}</span></label>{error && <p className="auth-message">{error}</p>}<div className="modal-footer"><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" disabled={saving || !task} onClick={() => void save()}><Check size={16} />{saving ? '保存中...' : '保存计划块'}</button></div></div></div>
}
...
function DailyCapacitySettings({ preferences, profiles, overrides, onSavePreferences, onAddProfile, onUpdateProfile, onDeleteProfile, onSaveOverride, onDeleteOverride }: { preferences: UserPreferences; profiles: CapacityProfile[]; overrides: DailyCapacityOverride[]; onSavePreferences: (input: UserPreferences) => Promise<void>; onAddProfile: (input: Omit<CapacityProfile, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>; onUpdateProfile: (id: string, input: Omit<CapacityProfile, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>; onDeleteProfile: (id: string) => Promise<void>; onSaveOverride: (input: Omit<DailyCapacityOverride, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>; onDeleteOverride: (id: string) => Promise<void> }) {
  const days = [1, 2, 3, 4, 5, 6, 0] as const
  const dayLabels = ['周日', '周一', '周二', '周三', '周四', '周五', '周六']
  const [base, setBase] = useState(preferences.baseDailyMinutes)
  const [load, setLoad] = useState(preferences.weeklyLoad)
  const [modeName, setModeName] = useState('')
  const [modeStart, setModeStart] = useState('')
  const [modeEnd, setModeEnd] = useState('')
  const [modeBase, setModeBase] = useState('')
  const [modePriority, setModePriority] = useState(0)
  const [editingMode, setEditingMode] = useState<string | null>(null)
  const [overrideDate, setOverrideDate] = useState('')
  const [overrideType, setOverrideType] = useState<'load' | 'minutes'>('load')
  const [overrideValue, setOverrideValue] = useState(100)
  const [overrideReason, setOverrideReason] = useState('')
  const [saveState, setSaveState] = useState<'idle' | 'saving' | 'saved' | 'error'>('idle')
  useEffect(() => { setBase(preferences.baseDailyMinutes); setLoad(preferences.weeklyLoad) }, [preferences])
  async function saveBase() { setSaveState('saving'); try { await onSavePreferences({ ...preferences, baseDailyMinutes: base, weeklyLoad: load }); setSaveState('saved') } catch { setSaveState('error') } }
  async function saveMode() { if (!modeName.trim() || !modeStart || !modeEnd) return; const input = { name: modeName.trim(), startDate: modeStart, endDate: modeEnd, baseDailyMinutes: modeBase === '' ? undefined : Number(modeBase), weeklyLoad: {}, priority: modePriority, enabled: true }; if (editingMode) await onUpdateProfile(editingMode, input); else await onAddProfile(input); setEditingMode(null); setModeName(''); setModeStart(''); setModeEnd(''); setModeBase(''); setModePriority(0) }
  async function saveOverride() { if (!overrideDate) return; await onSaveOverride({ date: overrideDate, loadPercent: overrideType === 'load' ? overrideValue : undefined, capacityMinutes: overrideType === 'minutes' ? overrideValue : undefined, reason: overrideReason.trim() || undefined }); setOverrideDate(''); setOverrideReason('') }
  return <div className="capacity-settings"><section className="panel settings-panel"><div className="panel-head"><div><h2>每日容量</h2><span className="panel-caption">容量设置只影响下一次重新排程，不会静默移动已有计划。</span></div></div><label>基础每日学习时长 <span>0 - 960 分钟</span><div className="range-row"><input type="range" min="0" max="960" step="15" value={base} onChange={e => setBase(Number(e.target.value))} /><strong>{Math.floor(base / 60)}h {base % 60}m</strong></div></label><div className="weekly-load-list">{days.map(day => <div className="weekly-load-row" key={day}><span>{dayLabels[day]}</span><input type="range" min="0" max="200" step="5" value={load[day]} onChange={e => setLoad({ ...load, [day]: Number(e.target.value) })} /><strong>{load[day]}% · {Math.floor(base * load[day] / 100 / 60)}h {Math.round(base * load[day] / 100) % 60}m</strong></div>)}</div><button className="button primary" disabled={saveState === 'saving'} onClick={() => void saveBase()}><Check size={16} />{saveState === 'saving' ? '保存中...' : '保存每日容量'}</button>{saveState === 'saved' && <p className="settings-success">已保存</p>}{saveState === 'error' && <p className="settings-error">保存失败，请检查数据库或网络后重试。</p>}</section><section className="panel settings-panel"><div className="panel-head"><div><h2>特殊模式</h2><span className="panel-caption">按日期生效；重叠时优先级更高的模式优先。</span></div></div><div className="inline-form capacity-mode-form"><input placeholder="模式名称，如考试周" value={modeName} onChange={e => setModeName(e.target.value)} /><input type="date" value={modeStart} onChange={e => setModeStart(e.target.value)} /><input type="date" value={modeEnd} onChange={e => setModeEnd(e.target.value)} /><input type="number" min="0" max="1440" step="15" placeholder="基础分钟（可选）" value={modeBase} onChange={e => setModeBase(e.target.value)} /><input type="number" min="-100" max="100" value={modePriority} onChange={e => setModePriority(Number(e.target.value))} /><button className="button primary" onClick={() => void saveMode()}>{editingMode ? '保存模式' : '新增模式'}</button></div><div className="settings-list">{profiles.length ? profiles.map(profile => <div className="settings-list-row" key={profile.id}><span>{profile.name} · {profile.startDate} - {profile.endDate} · 优先级 {profile.priority}</span><span><button className="text-btn" onClick={() => { setEditingMode(profile.id); setModeName(profile.name); setModeStart(profile.startDate); setModeEnd(profile.endDate); setModeBase(profile.baseDailyMinutes === undefined ? '' : String(profile.baseDailyMinutes)); setModePriority(profile.priority) }}>编辑</button><button className="text-btn danger" onClick={() => void onDeleteProfile(profile.id)}>删除</button></span></div>) : <p className="panel-caption">还没有特殊模式。</p>}</div></section><section className="panel settings-panel"><div className="panel-head"><div><h2>单日覆盖</h2><span className="panel-caption">可按比例或直接分钟数覆盖某一天。</span></div></div><div className="inline-form daily-override-form"><input type="date" value={overrideDate} onChange={e => setOverrideDate(e.target.value)} /><select value={overrideType} onChange={e => setOverrideType(e.target.value as 'load' | 'minutes')}><option value="load">负载比例</option><option value="minutes">直接分钟数</option></select><input type="number" min="0" max={overrideType === 'load' ? 200 : 1440} step={overrideType === 'load' ? 10 : 15} value={overrideValue} onChange={e => setOverrideValue(Number(e.target.value))} /><input placeholder="原因（可选）" value={overrideReason} onChange={e => setOverrideReason(e.target.value)} /><button className="button primary" onClick={() => void saveOverride()}>保存当天调整</button></div><div className="settings-list">{overrides.length ? overrides.map(item => <div className="settings-list-row" key={item.id}><span>{item.date} · {item.capacityMinutes !== undefined ? `${item.capacityMinutes} 分钟` : `${item.loadPercent}%`} {item.reason ? `· ${item.reason}` : ''}</span><button className="text-btn danger" onClick={() => void onDeleteOverride(item.id)}>删除</button></div>) : <p className="panel-caption">还没有单日覆盖。</p>}</div></section></div>
}

function SettingsView({ windowOpacity, onWindowOpacityChange, visualStyle, onVisualStyleChange, profile, availability, fixedEvents, preferences, capacityProfiles = [], dailyCapacityOverrides = [], onAddCapacityProfile = async () => undefined, onUpdateCapacityProfile = async () => undefined, onDeleteCapacityProfile = async () => undefined, onSaveDailyOverride = async () => undefined, onDeleteDailyOverride = async () => undefined, deepSeekKey, hasSavedDeepSeekKey, onDeepSeekKeyChange, onSaveDeepSeekKey, onDeleteDeepSeekKey, onSavePreferences, onSaveProfile, onImportSchedule, onAddAvailability, onUpdateAvailability, onDeleteAvailability, onAddFixedEvent, onUpdateFixedEvent, onDeleteFixedEvent, onAuthChange }: { windowOpacity: number; onWindowOpacityChange: (value: number) => void; visualStyle: VisualStyle; onVisualStyleChange:(value: VisualStyle)=>void; profile: UserProfile; availability: AvailabilityRule[]; fixedEvents: FixedEvent[]; preferences: UserPreferences; capacityProfiles?: CapacityProfile[]; dailyCapacityOverrides?: DailyCapacityOverride[]; onAddCapacityProfile?:(input: Omit<CapacityProfile, 'id'|'createdAt'|'updatedAt'>)=>Promise<void>; onUpdateCapacityProfile?:(id:string,input:Omit<CapacityProfile, 'id'|'createdAt'|'updatedAt'>)=>Promise<void>; onDeleteCapacityProfile?:(id:string)=>Promise<void>; onSaveDailyOverride?:(input: Omit<DailyCapacityOverride, 'id'|'createdAt'|'updatedAt'>)=>Promise<void>; onDeleteDailyOverride?:(id:string)=>Promise<void>; deepSeekKey: string; hasSavedDeepSeekKey: boolean; onDeepSeekKeyChange:(value:string)=>void; onSaveDeepSeekKey:()=>Promise<void>; onDeleteDeepSeekKey:()=>Promise<void>; onSavePreferences:(input: UserPreferences)=>Promise<void>; onSaveProfile:(input: UserProfile)=>Promise<void>; onImportSchedule:()=>void; onAddAvailability:(input:{weekday:number;startTime:string;endTime:string})=>Promise<void>; onUpdateAvailability:(id:string,input:{weekday:number;startTime:string;endTime:string})=>Promise<void>; onDeleteAvailability:(id:string)=>Promise<void>; onAddFixedEvent:(input:{title:string;startTime:string;endTime:string;recurrenceRule?:string})=>Promise<void>; onUpdateFixedEvent:(id:string,input:{title:string;startTime:string;endTime:string;recurrenceRule?:string})=>Promise<void>; onDeleteFixedEvent:(id:string)=>Promise<void>; onAuthChange:()=>Promise<void> }) {
  const [tab, setTab] = useState<'preferences'|'capacity'|'fixed'>('preferences')
  ...
  return <div className="page"><div className="page-head compact"><div><div className="eyebrow">偏好设置</div><h1>设置</h1><p className="subhead">让行更贴合你的学习节奏</p></div><button className="button primary" onClick={() => void saveSettings()}><Check size={16} />保存设置</button></div><div className="settings-layout"><div className="settings-nav"><button className={tab === 'preferences' ? 'selected' : ''} onClick={() => setTab('preferences')}>学习偏好</button><button className={tab === 'capacity' ? 'selected' : ''} onClick={() => setTab('capacity')}>每日容量</button>
