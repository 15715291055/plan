...
function App() {
  ...
  async function addTask(title: string, minutes: number, course: string, strategy: ReplanStrategy = 'minimal_change', deadlineIso?: string | null, difficulty?: number, type?: string, completionMode: TaskCompletionMode = 'single_day', spreadDays?: number) {
    try {
      const created = await createTaskRecord({ title, minutes, course: course || '未分类', deadlineIso, difficulty, type, source: 'temporary', completionMode, spreadDays })
  ...
  async function editTask(id: string, title: string, minutes: number, course: string, deadlineIso?: string | null, difficulty?: number, type?: string, completionMode: TaskCompletionMode = 'single_day', spreadDays?: number) {
    const previous = tasks
    setTasks(current => current.map(task => task.id === id ? { ...task, title, minutes, course: course || '未分类', deadlineIso: deadlineIso ?? undefined, deadline: deadlineIso ? new Date(deadlineIso).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '未设置', difficulty: difficulty ? difficulty >= 4 ? '较难' : difficulty >= 3 ? '中等' : '简单' : task.difficulty, type: type ?? task.type, completionMode, spreadDays: completionMode === 'spread_days' ? spreadDays : undefined } : task))
    try { await updateTaskRecord(id, { title, minutes, course: course || '未分类', deadlineIso, difficulty, type, completionMode, spreadDays }); setEditingTask(null); setToast(dataSource === 'supabase' ? '任务已更新到云端' : '任务已更新') } catch (error) { setTasks(previous); setToast(error instanceof Error ? `保存失败：${error.message}` : '保存失败，请重试') }
  }
